  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.paymentMethod === 'Razorpay') {
          ...
          const message = `*New Room Inquiry - Hotel Bhopal Inn*%0A%0A` +
              `*Guest Name:* ${formData.firstName} ${formData.lastName}%0A` +
              `*Room Type:* ${selectedRoom?.title}%0A` +
              `*Check-In:* ${formData.checkInDate}%0A` +
              `*Check-Out:* ${formData.checkOutDate}%0A` +
              `*Plan:* ${formData.plan}%0A` +
              `*Total Amount:* ₹${finalPrice}%0A` +
              `*Payment:* Pay at Hotel%0A%0A` +
              `Please confirm my booking.`;
          
          const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${message}`;

          setStatus('Booking Request Sent Successfully! Redirecting to WhatsApp for confirmation...');
          setLoading(false);
