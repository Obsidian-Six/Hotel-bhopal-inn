...
const BanquetPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const whatsappNumber = "916267276957";
      const message = `*New Banquet Inquiry - Hotel Bhopal Inn*%0A%0A` +
          `*Name:* ${formData.fullName}%0A` +
          `*Mobile:* ${formData.mobileNumber}%0A` +
          `*Event Type:* ${formData.eventType}%0A` +
          `*Date:* ${formData.preferredDate}%0A` +
          `*Guests:* ${formData.noOfGuests}%0A` +
          `*Meal:* ${formData.mealRequirement}%0A` +
      ...
      setSubmitMessage({ 
        type: 'success', 
        text: 'Thank you! Redirecting to WhatsApp for final confirmation...' 
      });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white overflow-x-hidden">
      ...
      <a href="https://wa.me/916267276957" className="fixed bottom-10 right-10 z-[300] w-16 h-16 bg-[#25D366] rounded-full flex items-center justify-center text-white shadow-2xl hover:scale-110 transition-transform"><MessageCircle size={32} /></a>
    </div>
  );
};
